...
function set_custom_edit_listing_columns($columns) {
    ...
    $columns['listing_price'] = __( 'Listing Price', 'hus_companion' );
    $columns['listing_area'] = __( 'Listing Area', 'hus_companion' );
...
function custom_listing_column( $column, $post_id ) {
    ...
    switch ( $column ) {
        ...
        case 'listing_price' :
            echo get_post_meta( $post_id , '_hus_listing_price' , true ); 
            break;

        case 'listing_area' :
            echo get_post_meta( $post_id , '_hus_listing_area' , true ); 
            break;
...
if ( ! function_exists( 'search_prop_form_datas' ) ) {
	function search_prop_form_datas() {
		...
		if ( $search_text == '' || $search_category == '' || $search_location == '' || $search_area_from == '' || $search_area_to == '' || $price_min == '' || $price_max == '' ) {
			...
			$response['message']  = __( 'Sorry! Empty field is not allowed.', 'hus-companion' );
		} else {
			$item = 0;
